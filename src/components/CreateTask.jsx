...
const CreateTask = ({ tasks, setTasks }) => {
  const [task, setTask] = useState({
    id: "",
    name: "",
    status: "todo" //can also be inprogeass or closed
  });

  const handleSubmit = (e) => {
    e.preventDefault();
    if(task.name.length < 3) return toast.error("A task must have more than 3 characters");
    if(task.name.length > 100) return toast.error("A task must have less than 100 characters");
    setTasks((prev) => {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      />
      <button
        className='bg-cyan-500 rounded-md px-4 h-12 
      text-white'
        type='submit'
      >
        Create
